...
- **ToteatMessageInputBar**: Standalone message input: a multi-line `ToteatTextField` (68.dp minimum height, 12.dp corners, top-aligned text and placeholder) plus a trailing `ToteatSendIconButton`, vertically centered against the field. The soft keyboard exposes an `ImeAction.Send` that triggers the same `onSendClick`. Derived test tags: `_field`, `_send`.
...
- **ToteatTextField**: New optional parameters `minHeight: Dp = 50.dp` and `shape: Shape = RoundedCornerShape(8.dp)`, forwarded to `ToteatTextFieldLayout` (which gained the same parameters and now uses `heightIn(min = ...)` instead of a fixed 50.dp height). With `singleLine = false` the text and the placeholder are top-aligned instead of centered, so a taller multi-line field fills from the top — this is what `ToteatMessageInputBar` uses. Existing call sites are unaffected: the defaults reproduce the previous fixed 50.dp / 8.dp-corner single-line field.
